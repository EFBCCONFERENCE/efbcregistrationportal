...
const migrateTierTracking = async () => {
    try {
        const dbNameRows = await databaseService.query('SELECT DATABASE() as db');
        const dbName = dbNameRows[0]?.db;
        if (!dbName)
            return;
        const regCols = await databaseService.query('SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?', [dbName, 'registrations']);
        const has = (name) => regCols.some((c) => c.COLUMN_NAME === name);
        const alter = [];
        const add = (col, type) => {
            if (!has(col))
                alter.push(`ADD COLUMN \`${col}\` ${type}`);
...
const migrateRibbonsFeature = async () => {
    try {
        const dbNameRows = await databaseService.query('SELECT DATABASE() as db');
        const dbName = dbNameRows[0]?.db;
        if (!dbName)
        ...
        const registrationCols = await databaseService.query('SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?', [dbName, 'registrations']);
        const hasEventRibbons = eventCols.some((c) => c.COLUMN_NAME === 'ribbons');
        const hasRegistrationRibbons = registrationCols.some((c) => c.COLUMN_NAME === 'ribbons');
        if (!hasEventRibbons) {
...
const migrateEventEditPolicy = async () => {
    try {
        const dbNameRows = await databaseService.query('SELECT DATABASE() as db');
        const dbName = dbNameRows[0]?.db;
        if (!dbName)
            return;
        const eventCols = await databaseService.query('SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?', [dbName, 'events']);
        const hasAllowAttendeeEdits = eventCols.some((c) => c.COLUMN_NAME === 'allow_attendee_edits');
